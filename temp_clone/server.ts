...
`;
  ...
  app.post('/api/generate-prompt', async (req, res) => {
    ...
    try {
      ...
      const cacheInput = `${mimeType}_${base64Data.length}_${contentHash}_${model}_${analysisMode}_${targetAI}_${segmentDuration}_${includeActions}_${includeVoiceOver}_${includeCinematics}_${sourceCaption ? sourceCaption.slice(0, 50) : ''}`;
      ...
      const seoCaptionGuide = sourceCaption && sourceCaption.trim()
        ? `\n### INSTRUKSI CAPTION & HASHTAG SEO VIRAL (SANGAT PENTING):
Terdapat CAPTION ASLI dari sumber video TikTok input:
"""${sourceCaption.trim()}"""

Tugas Anda: Analisis video ini beserta caption sumber di atas. Buatlah CAPTION SEJENIS YANG JAUH LEBIH RELEVAN, MENGANDUNG SEO VIRAL ALGORITMA TIKTOK / REELS / SHORTS FYP, Hook Kuat & Menggigit, Storytelling/Benefit Persuasif, dan Call to Action (CTA) yang jelas. Tingkatkan relevansi dan daya tarik SEO-nya agar performa video naik drastis!
Sertakan juga 10-15 HASHTAG VIRAL & SEO SEARCH (kombinasi broad tags, niche tags spesifik produk/konten, dan high-intent search terms).`
        : `\n### INSTRUKSI CAPTION & HASHTAG SEO VIRAL (SANGAT PENTING):
Berdasarkan konten visual dan suara video ini, buatlah CAPTION VIRAL SEO TIKTOK / REELS / SHORTS FYP yang sangat relevan (Hook kuat di kalimat pertama, Storytelling / Benefit persuasif, CTA jelas), serta 10-15 HASHTAG VIRAL & SEO SEARCH yang relevan dengan topik video.`;
      ...
      if (segmentDuration !== 'auto') {
        ...
        promptText = `Anda adalah AI Video Prompt Engineer, Sinematografer & Social Media SEO Specialist Kelas Dunia.
${analysisDepthGuide}
...
Di akhir output, Anda WAJIB menyertakan bagian Caption & Hashtag berikut ini secara lengkap dan jelas:

### 📱 CAPTION & HASHTAG VIRAL SEO (SIAP POSTING)

**Caption SEO FYP:**
[Tulis kalimat caption lengkap dalam Bahasa Indonesia yang dioptimalkan untuk SEO FYP TikTok/Reels/Shorts, copywriting memikat, dan CTA]

**Hashtags Viral & SEO Search:**
#HashtagViral1 #HashtagNiche2 #HashtagSEO3 #FYP #Trending #...`;
      } else {
        promptText = `Anda adalah AI Video Prompt Engineer, Sinematografer & Social Media SEO Specialist Kelas Dunia.
${analysisDepthGuide}
...
${promptText}`
      ...
      const result = await callGeminiWithFallback(userSelectedModel, promptPayload, customApiKey, clientAccessCode, undefined, 'Video to Prompt AI Splitter');
...
DETEKSI MODE INPUT:
1. JIKA INPUT BERISI BANYAK KLIP SEKALIGUS (MULTI-CLIP / BATCH SCENE BREAKDOWN):
   (Misalnya terdapat format "[00:00 - 00:05] Klip 1", "Klip 1:", "Segmen 1", atau rincian adegan 1 s/d N dari TikTok Shop / Ide Konten / Video to Prompt):
   - Anda WAJIB mempertahankan konsistensi identitas karakter, wajah, talent, lighting palette, dan produk di SEMUA KLIP.
   - Hasilkan prompt foto terpisah untuk SETIAP KLIP secara berurutan lengkap dengan format di bawah.
   - Gunakan format header klip: "### 🎬 KLIP [X] ([TIMESTAMP]) — [JUDUL / FOKUS ADEGAN]" untuk setiap klip.
   - Setiap klip memiliki blok prompt code (\`\`\`text ... \`\`\`) yang berdiri sendiri dan SIAP SALIN (1-click copy).
...
### 🎬 KLIP 1 (00:00 - 00:05) — [HOOK VISUAL MAKSIMAL]
\`\`\`text
[Master Shot]: Hyper-realistic ${photoStyle} photography, TikTok hook scene 1...
[Subject & Identity]: ...
[Scene Context & Environment]: ...
[Lighting & Physics]: ...
[Camera & Optics]: ...
[Texture & Quality]: 8k UHD resolution, ${aspectRatio} --style raw --v 6.1
\`\`\`

### 🎬 KLIP 2 (00:05 - 00:10) — [PROBLEM AGITATION & DETAIL]
\`\`\`text
[Master Shot]: Hyper-realistic ${photoStyle} photography, scene 2...
[Subject & Identity]: ... (same character/product identity)
[Scene Context & Environment]: ...
[Lighting & Physics]: ...
[Camera & Optics]: ...
[Texture & Quality]: 8k UHD resolution, ${aspectRatio} --style raw --v 6.1
...
(Lanjutkan untuk seluruh klip yang ada pada konsep input pengguna hingga tuntas)

---

### 🔍 ANALISIS MENDALAM RELEVANSI KONTEN & KONSISTENSI VISUAL BATCH
- **🎯 Konsistensi Karakter & Produk**: [Penjelasan bagaimana identitas subjek dan produk dijaga konsisten dari klip 1 hingga klip akhir].
- **⚡ TikTok Hook Pacing & Transisi Visual**: [Strategi pergantian komposisi shot antar klip agar penonton tidak bosan].
- **💡 Rekomendasi Prompt Generator**: [Saran model gambar terbaik seperti Midjourney v6.1, Flux.1, atau Ideogram v2].`;
      ...
      const result = await callGeminiWithFallback(userSelectedModel, promptPayload, customApiKey, clientAccessCode, undefined, 'Photo Prompt 8K Ultra HD');
  ...
  app.post('/api/generate-content-ideas', async (req, res) => {
    ...
    try {
      ...
      const cacheInput = `content_ideas_v3_${mimeType}_${sampleData}_${model}_${contentType}_${tone}_${maxDuration}_${segmentDuration}_${targetAI}_${aeoQueryMode}_${enableBigSound}_${enableTextOverlay}_${refImgSample}_${userSeedSample}_${totalIdeas}`;
      const cacheKey = crypto.createHash('sha256').update(cacheInput).digest('hex');
...
Hasilkan HANYA 1 paragraf padat berbahasa Inggris yang mendeskripsikan secara jelas ciri khas subjek/produk utama di gambar ini.`;
      ...
      // =========================================================================
      logger.info(`[Content Ideas Stage 2] Menghasilkan ${totalIdeas} ide konten grounded...`);
...
`;
      ...
      const stage2Result = await callGeminiWithFallback(userSelectedModel, stage2Payload, customApiKey, clientAccessCode, undefined, 'Generator Ide Konten (AEO)');
...
BERIKUT HASIL ${totalIdeas} IDE KONTEN YANG DI-GENERATE TAHAP 2:
"""
${finalOutputText}
"""
...
- Jika ada kesalahan/halusinasi fatal, perbaiki kalimat aksi/produk tersebut agar 100% sesuai dengan fakta visual Tahap 1, lalu kembalikan teks utuh yang sudah diperbaiki.`,
      ...
      try {
        ...
        if (validatedResult?.text && (validatedResult.text.includes('IDE KONTEN VIRAL') || validatedResult.text.includes('IDE 1:'))) {
...
`;
      ...
      const geminiResult = await callGeminiWithFallback(userSelectedModel, payload, customApiKey, clientAccessCode, undefined, 'TikTok Shop to Ideas');
  ...
  app.post('/api/verify-access-code', async (req, res) => {
    ...
    if (banCheck.banned) {
      ...
      try {
        ...
        broadcastLiveEvent({ type: 'audit_logs_updated' });
      } catch (e) {}
