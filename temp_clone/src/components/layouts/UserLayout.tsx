...
export default function UserLayout({ session, onLogout, onGoToAdmin }: UserLayoutProps) {
  ...
  const handleSendToPhotoPrompt = (text: string, options?: { negativePrompt?: string; referenceImage?: File; autoGenerate?: boolean }) => {
  ...
  const generatePrompt = async () => {
    ...
    try {
      ...
      const effectiveModel = analysisMode === 'deep' ? 'gemini-3.1-pro-preview' : 'gemini-3.6-flash';
      ...
      setActiveModelUsed(data.modelUsed || effectiveModel);
        ...
        <header className="bg-white border-b border-slate-200/80 px-4 sm:px-6 py-3 flex items-center justify-between gap-3 shrink-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <div 
              ...
              className="px-3 py-1 rounded-full bg-[#7c3aed] text-white font-bold text-xs flex items-center gap-1.5 cursor-pointer shadow-2xs hover:bg-[#6d28d9] transition-colors"
            ...
            <div className="max-w-5xl mx-auto space-y-6">
              ...
              <div className="bg-white border border-slate-200/80 rounded-2xl p-6 sm:p-8 shadow-sm space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  {/* Option 2: Fast analysis vs Deep analysis Mode */}
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <label className="text-xs font-bold text-slate-800 uppercase tracking-wider flex items-center gap-1.5">
                        <Sparkles className="w-3.5 h-3.5 text-amber-500" />
                        Mode Analisis Video AI
                      </label>
                      <span className="text-[11px] text-slate-500 font-medium">
                        {analysisMode === 'deep' ? 'Ultra Sinematik & Detail' : 'Eksekusi Cepat & Responsif'}
                      </span>
                    </div>
                    <div className="grid grid-cols-2 gap-1.5 p-1 bg-slate-100 border border-slate-200 rounded-xl text-xs font-medium">
                      <button
                        type="button"
                        onClick={() => {
                          setAnalysisMode('fast');
                          learningSync.track('analysis_mode_selected', { mode: 'fast' });
                        }}
                        className={`py-2 px-3 rounded-lg text-center transition-all flex items-center justify-center gap-2 cursor-pointer ${
                          analysisMode === 'fast'
                            ? 'bg-[#5b50e5] text-white font-bold shadow-2xs'
                            : 'text-slate-600 hover:text-slate-900'
                        }`}
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          learningSync.track('analysis_mode_selected', { mode: 'deep' });
                        }}
                        className={`py-2 px-3 rounded-lg text-center transition-all flex items-center justify-center gap-2 cursor-pointer ${
                          analysisMode === 'deep'
                            ? 'bg-[#5b50e5] text-white font-bold shadow-2xs'
