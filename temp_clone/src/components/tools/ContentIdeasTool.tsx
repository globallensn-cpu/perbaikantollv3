...
interface ContentIdeasToolProps {
  ...
  onSendToPhotoPrompt?: (text: string, options?: { negativePrompt?: string; referenceImage?: File; autoGenerate?: boolean }) => void;
...
export default function ContentIdeasTool({
  ...
  const [numIdeas, setNumIdeas] = useState<number>(5);
  ...
  const handleGenerateIdeas = async () => {
    ...
    const progressInterval = setInterval(() => {
      ...
      if (currPct < 40) {
        ...
        setProgressStep(`Tahap 2: Meracik ${numIdeas} Ide Konten Grounded & Dialog Natural Anti-AI-Slop...`);
      } else {
    ...
    try {
      ...
      setProgressStep(`Selesai membuat ${numIdeas} ide konten grounded!`);
      ...
      saveHistoryItem({
        ...
        title: displayTitle,
        subtitle: `${numIdeas} Ide Konten • Durasi: ${maxDuration}s (Pecah ${segmentDuration}s) • Tipe: ${contentType.toUpperCase()}`,
        data: {
          prompt: data.result,
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto">
      ...
      <div className="bg-white border border-slate-200/80 rounded-2xl p-6 sm:p-8 shadow-sm space-y-6">
        ...
        {/* Configurations Row: Duration, Segment Split, AEO Mode & Number of Ideas */}
        <div className="pt-4 border-t border-slate-100 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          {/* Number of Ideas Selector (1, 2, 3, 4, or 5) */}
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-slate-800 uppercase tracking-wider flex items-center justify-between">
              ...
              <span className="text-[11px] text-[#5b50e5] font-bold">{numIdeas} Ide Dipilih</span>
            </label>
            <div className="grid grid-cols-5 gap-1">
              {[1, 2, 3, 4, 5].map((num) => (
                <button
                  ...
                  className={`h-11 rounded-xl text-xs font-bold transition-all border cursor-pointer flex items-center justify-center ${
                    numIdeas === num
                      ? 'bg-[#5b50e5] text-white border-[#5b50e5] shadow-xs font-extrabold ring-2 ring-[#5b50e5]/20'
                      : 'bg-white text-slate-600 border-slate-200 hover:border-slate-300 hover:bg-slate-50'
                  }`}
                  title={`Hasilkan ${num} Ide Konten`}
                >
                  {num}
                </button>
              ))}
          ...
          >
            {isGenerating ? (
              <>
                ...
                <span>Meracik {numIdeas} Ide Konten, Pecah Prompt {maxDuration}s & Hashtag FYP...</span>
              </>
            ) : (
              <>
                <Wand2 className="w-5 h-5 group-hover:rotate-12 transition-transform" />
                <span>Hasilkan {numIdeas} Ide Konten, Prompt Adegan ({maxDuration}s) & Hashtag Relevan</span>
              </>
            )}
          </button>
        </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm sm:text-base font-bold text-slate-900 flex items-center gap-2 flex-wrap">
                    <span>{parsedIdeas.length || numIdeas} Ide Konten TikTok & Hashtag SIAP PAKAI</span>
                    {activeModelUsed && (
                          ...
                          <div className="flex items-center gap-2 flex-wrap self-end sm:self-auto">
                            {onSendToPhotoPrompt && idea.clips.length > 0 && (
                              <button
                                type="button"
                                onClick={() => {
                                  // Construct complete batch text of all clips
                                  const clipsBatchText = `KONSEP IDE KONTEN BATCH PROMPT FOTO (${idea.clips.length} KLIP):\n` +
                                    `Judul Ide: ${idea.title}\n\n` +
                                    idea.clips.map(c => `### [${c.timeRange}] Klip ${c.id}: ${c.title}\nDeskripsi Adegan Visual & Voice Over:\n${c.actionAndVO || ''}\n${c.aiPrompt ? `\nPrompt AI Video:\n${c.aiPrompt}` : ''}`).join('\n\n---\n\n');
                                ...
                                title="Generate otomatis seluruh prompt foto untuk semua klip dalam ide ini sekaligus"
                              >
