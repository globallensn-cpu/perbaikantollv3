...
interface SplitPromptViewerProps {
  ...
  onSendToPhotoPrompt?: (text: string, options?: { autoGenerate?: boolean }) => void;
}
...
export function parseClipSegments(rawText: string): ClipSegment[] {
  ...
  // Remove the caption & hashtags footer section before parsing video clips so it doesn't get treated as a clip
  const clipsPart = rawText.split(/(?:###|\*\*)\s*(?:📱|🔥|✨)?\s*CAPTION\s*(?:&|DAN)?\s*HASHTAG/i)[0];

  // Split on headings that indicate clips or segments
        ...
        <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
          ...
          {segments.length > 0 && onSendToPhotoPrompt && (
            <button
              type="button"
              onClick={() => {
                const batchClipsText = `KONSEP VIDEO TO PROMPT BATCH PROMPT FOTO (${segments.length} KLIP):\n` +
                ...
                onSendToPhotoPrompt(batchClipsText, { autoGenerate: true });
              }}
