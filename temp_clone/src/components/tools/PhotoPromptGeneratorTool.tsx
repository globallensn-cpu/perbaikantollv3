...
export default function PhotoPromptGeneratorTool({ initialConcept, initialNegativePrompt, initialReferenceImage, autoGenerate }: PhotoPromptGeneratorToolProps) {
  ...
  useEffect(() => {
    ...
    if (autoGenerate && combinedText && !hasAutoGeneratedRef.current) {
      ...
      setTimeout(() => {
        handleGeneratePhotoPromptWithText(combinedText, initialNegativePrompt, initialReferenceImage);
      }, 200);
    }
  }, [initialConcept, initialNegativePrompt, initialReferenceImage, autoGenerate]);
  ...
  const handleGeneratePhotoPromptWithText = async (overrideText?: string, overrideNeg?: string, overrideRefImg?: File | null) => {
    ...
    reportActiveGenerationStatus(activeId, 'generating', `Prompt Foto Multi-Klip (${targetGenerator})`);
