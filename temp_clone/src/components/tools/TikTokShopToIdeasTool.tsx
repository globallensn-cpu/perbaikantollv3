        ...
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
          ...
          <div className="space-y-1.5 sm:space-y-2">
            <label className="block text-xs sm:text-sm font-bold text-slate-900 flex flex-wrap items-center justify-between gap-1">
              ...
              <span className="text-slate-400 font-normal text-[11px] sm:text-xs">Optional</span>
            </label>

            <textarea
              rows={3}
              placeholder="E.g., Key features, target audience, specific pain points to address..."
              value={productDetails}
              onChange={(e) => setProductDetails(e.target.value)}
              className="w-full p-3 sm:p-3.5 rounded-xl border border-slate-200 text-xs sm:text-sm focus:ring-2 focus:ring-[#7c3aed]/20 focus:border-[#7c3aed] outline-none bg-white text-slate-900 font-medium resize-none transition-all placeholder:text-slate-400"
            />
          </div>
          ...
          <div className="pt-2 grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4">
            ...
            <div className="space-y-1.5">
              ...
              >
                <option value="15">15 Detik (Short FYP)</option>
                <option value="30">30 Detik (Standard FYP)</option>
                <option value="60">60 Detik / 1 Menit (Utama)</option>
          ...
          {/* ROW: ANALYSIS MODE & NUMBER OF IDEAS */}
                ...
                {[1, 2, 3, 4, 5].map((num) => (
                  <button
                    ...
                    className={`w-9 h-9 rounded-xl text-xs font-bold transition-all border cursor-pointer flex items-center justify-center ${
                      numIdeas === num
                        ? 'bg-[#7c3aed] text-white border-[#7c3aed] shadow-2xs font-extrabold ring-2 ring-[#7c3aed]/20'
                            ...
                            {onSendToPhotoPrompt && idea.clips.length > 0 && (
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  const clipsBatchText = `KONSEP TIKTOK SHOP BATCH PROMPT FOTO (${idea.clips.length} KLIP):\n` +
                                    `Produk/Judul: ${idea.title}\n\n` +
