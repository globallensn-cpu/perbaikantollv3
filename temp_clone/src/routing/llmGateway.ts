...
import { dbGetApiKeys, dbSaveApiKeys, dbAddApiKeyLog } from '../db/dbService';
...
// Global Model Priorities and Fallback Hierarchy
export const GATEWAY_MODELS_HIERARCHY: string[] = [
  'gemini-3.7-flash',
  'gemini-3.6-flash',
...
function normalizeGeminiModel(inputModel?: string): string {
  ...
  if (m === 'gemini-2.5-pro' || m === 'gemini-2.0-pro' || m === 'gemini-1.5-pro') return 'gemini-2.5-pro';
  if (m === 'gemini-2.5-flash' || m === 'gemini-2.0-flash' || m === 'gemini-1.5-flash') return 'gemini-3.6-flash';
  if (m === 'gemini-2.5-flash-lite' || m === 'gemini-2.0-flash-lite' || m === 'gemini-1.5-flash-lite') return 'gemini-3.1-flash-lite';
  return inputModel || 'gemini-3.7-flash';
}
...
export class LLMGateway {
  ...
  private constructor() {
    // Initial health check timer (every 30 seconds recover cooled down keys)
    setInterval(() => {
  ...
  public async getAvailableKeyCandidates(
    ...
    const now = Date.now();
    const candidates: { key: string; keyId: string; source: 'user_custom' | 'admin_pool' | 'env' }[] = [];
    ...
    // If user provided valid custom keys, prioritize them
    ...
    // Remove duplicates
    const uniqueMap = new Map<string, { key: string; keyId: string; source: 'user_custom' | 'admin_pool' | 'env' }>();
    ...
    // Weighted Load Balancing Sort:
    // 1. Filter out revoked keys and keys currently in cooldown
    // 2. Sort by: Lowest Active In-flight Requests -> Fewest Consecutive Errors -> Lowest Latency -> Least Recently Used
    const sorted = uniqueCandidates
      ...
      .sort((a, b) => {
        ...
        // In-flight active load penalty (Weight = 100)
        const loadDiff = (stateA.activeRequests - stateB.activeRequests) * 100;
        if (loadDiff !== 0) return loadDiff;

        // Consecutive errors penalty (Weight = 50)
    ...
    // Fallback: If all keys are in cooldown, pick the key with the earliest cooldown expiration
   ...
   * cascades across candidate models, trips circuit breakers on 429/503 errors, and maintains 99.9% uptime.
   */
  public async execute(options: LLMGatewayRequestOptions): Promise<LLMGatewayResponse> {
    ...
    const primaryModel = normalizeGeminiModel(options.model);

    // Build intelligent cascading model list
    const candidateModels = Array.from(
      new Set([primaryModel, ...GATEWAY_MODELS_HIERARCHY])
    ).filter(Boolean);
    ...
    if (keyCandidates.length === 0) {
      ...
      throw new Error(
        'Tidak ada API Key Gemini yang aktif atau tersedia. Silakan tambahkan API Key di Pengaturan Anti Limit.'
      );
    }
    ...
    for (let kIdx = 0; kIdx < keyCandidates.length; kIdx++) {
      const candidate = keyCandidates[kIdx];
      ...
      for (const targetModel of candidateModels) {
        ...
        while (modelAttempts < maxModelAttempts) {
          ...
          try {
            logger.info(
              `[LLM Gateway Request] Key #${kIdx + 1}/${keyCandidates.length} (${keyMasked}), Model: ${targetModel}, Attempt: ${modelAttempts}...`
            );
            ...
            return {
              ...
              tierUsed: targetModel.includes('pro')
                ? 'flagship'
                : targetModel.includes('lite')
                ? 'tier3'
                : 'tier2',
            };
          } catch (err: any) {
            ...
            if (isDeadKey) {
              ...
              keyState.lastErrorReason = 'API Key Invalid / Revoked (403)';
              this.recordKeyRevocation(activeKey, keyId).catch(() => {});
              logger.error(`[LLM Gateway Auto-Prune] Key ${keyMasked} is revoked/invalid. Moving to next candidate key.`);
              break; // break model loop for this key
            }

            if (isRateLimitOrQuota) {
              this.metrics.rateLimitHits++;
              this.metrics.circuitBreakerTrips++;
              // Trip circuit breaker with 5-minute cooldown
              const cooldownMs = 5 * 60 * 1000;
              keyState.status = 'rate_limited';
              keyState.cooldownUntil = Date.now() + cooldownMs;
              keyState.lastErrorReason = 'Rate Limit / Quota Exceeded (429)';

              this.recordRateLimitCooldown(activeKey, keyId, cooldownMs).catch(() => {});
              logger.warn(
                `[LLM Gateway Circuit Breaker] Rate limit triggered on ${keyMasked}. Isolated for 5 minutes.`
              );
              break; // break model loop for this key and switch to next key candidate immediately
            }

            if (isServerError && modelAttempts < maxModelAttempts) {
              // Exponential backoff with jitter before next retry on the same key
              const jitter = Math.floor(Math.random() * 300) + 150;
              const delay = Math.min(2000, 400 * Math.pow(2, modelAttempts - 1) + jitter);
