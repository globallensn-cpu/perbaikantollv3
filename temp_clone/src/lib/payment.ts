...
export async function uploadPaymentProof(
  ...
  try {
    const res = await setDoc(doc(db, 'transactions', cleanId), targetTrx).catch(console.error);
    if (res.ok) {
    ...
    console.warn('[Payment] Failed sending proof to backend:', err);
  }
...
export function approveTransaction(trxId: string): Transaction | null {
  ...
  try {
    import('./admin/clients').then(({ getClients, saveClients, calculateClientStatus }) => {
      ...
      const newClient = {
        id: existingClientIdx >= 0 ? clients[existingClientIdx].id : `cli_${Date.now()}`,
      ...
      if (existingClientIdx >= 0) {
        clients[existingClientIdx] = { ...clients[existingClientIdx], ...newClient };
      } else {
  ...
  // Sync to server
  setDoc(doc(db, 'transactions', trxId), approvedTrx).catch(console.error);
...
export function rejectTransaction(trxId: string, reason: string): Transaction | null {
  ...
  // Sync to server
  setDoc(doc(db, 'transactions', trxId), rejectedTrx).catch(console.error);
